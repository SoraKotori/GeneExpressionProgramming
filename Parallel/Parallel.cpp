...
private:
	...
	decltype(auto) ThreadWait()
	{
		std::unique_lock<std::mutex> _Lock(_Mutex);

		auto _ThisGeneration = _Generation;
		_Condition.wait(_Lock, [&] { return _ThisGeneration != _Generation; });
	}

	decltype(auto) ThreadComputing(Size _FirstIndex, Size _LastIndex)
	{
		for (ThreadWait();; ThreadWait())
		{
	...
	decltype(auto) ThreadInitialize(Size _PopulationSize, unsigned int _ThreadCount = std::thread::hardware_concurrency())
	{


		Vector<std::thread> _VectorThread(_ThreadCount);
		std::generate(std::begin(_VectorThread), std::end(_VectorThread), [this]
		{
			std::thread _Thread;
			return _Thread;
		});

		return _VectorThread;
	}
...
public:
	GeneExpressionProgramming(
		...
		_ThreadPool(ThreadInitialize(_PopulationSize))
	{
	...
	{
		return ThreadComputing();
	}

	decltype(auto) GetBestChromosome()
	{
		// Need const
		return *std::min_element(std::begin(_Parent), std::next(std::begin(_Parent), _ElitismSize), ComparisonsPair);
	}
