...
typedef struct  //Enviroment configure.
{
  int popSize;
  int headLen;
  double crossRate;
...
int ReadConfig(char* fileName, G_Config* gConfig)
{
  std::fstream cfgFile;
  std::string tmpStr, subStr;
  std::stringstream splitStr, valueStr;
  cfgFile.open(fileName, std::ios::in);
  int cfgType;
  if(!cfgFile)
  {
    std::cout << "Config file" << fileName << "does not exist!" << std::endl;
    return G_FILE_ERR;
  }
  else
  {
    while(std::getline(cfgFile, tmpStr))
    {
      splitStr.str("");
      splitStr.clear();
      splitStr.str(tmpStr);
      while( std::getline(splitStr, subStr, '=') )
      {
        if(subStr == "PopulationSize")
          cfgType = POPSIZE;
        else if(subStr == "HeadLength")
          cfgType = HEADLEN;
        else if(subStr == "CrossoverRate")
          cfgType = CROSSRATE;
        else if(subStr == "MutationRate")
          cfgType = MUTRATE;
        else
        {
          valueStr.str("");
          ...
          {
            case POPSIZE:
                valueStr >> gConfig->popSize;
                std::cout << "Popsize = " << gConfig->popSize << std::endl;
                break;
            case HEADLEN:
                valueStr >> gConfig->headLen;
                std::cout << "HeadLength = " << gConfig->headLen << std::endl;
                break;
            case CROSSRATE:
                valueStr >> gConfig->crossRate;
                std::cout << "CrossoverRate = " << gConfig->crossRate << std::endl;
                break;
            case MUTRATE:
                valueStr >> gConfig->mutationRate;
                std::cout << "MutationRate = " << gConfig->mutationRate << std::endl;
                break;
            default:
...
decltype(auto) WriteFile_csv(_Vector&& _FitnessVector, _Path&& _FilePath, _SizeType _Size)
{
	auto _File = std::ofstream(_FilePath);
	std::transform(std::begin(_FitnessVector), std::end(_FitnessVector), std::begin(_FitnessVector), std::bind(std::divides<>(), std::placeholders::_1, _Size));
...
int main(int argc, char** argv)
{
	if(argc != 3)
	{
		std::cout << "Usage: " << argv[0] << " [cfgFile] [csvFile]" <<std::endl;
		return 0;
	}

	G_Config gConfig;
	int ret;
	ret = ReadConfig(argv[1], &gConfig);
	if(ret != 0)
	{
	...
	auto _TrainingData = ReadFile_csv<float>(argv[2]);
	std::size_t _PopulationSize = gConfig.popSize;
	std::size_t _HeadLength = gConfig.headLen;
	std::size_t _ElitismSize = _PopulationSize / 3;
	double _CrossoverRate = gConfig.crossRate;
	double _MutationRate = gConfig.mutationRate;
	...
	std::size_t _PerturbingCount = 10;
	std::size_t _PerturbingSize = _ElitismSize;
	std::vector<float> _FitnessVector(_IteratorCount);
	...
	for (decltype(_IteratorCount) _Iterator = 0; _Iterator < _IteratorCount; _Iterator++)
	{
		...
		_FitnessVector[_Iterator] = _Fitness;

		if (0 == _Fitness)
		{
			_FitnessVector.resize(_Iterator + 1);
			break;
		}
	...
	std::cout << "IteratorCount : " << _FitnessVector.size() << std::endl;
	...
	WriteFile_csv<float>(_FitnessVector, "Fitness.csv", _TrainingData.size());
